...
public class Individuals {
	 ...
	 * @param m 変異率
	 */
	public Individuals(int n, int m, int p, double c, double p_m) {
		mPopCount = p;
		mCrossRate = c;
		mIndivs = new Chromosome[mPopCount];
		mElite = new Chromosome(n, m, p_m);
		updated = false;

		for (int i = 0; i < mPopCount; i++) {
			mIndivs[i] = new Chromosome(n, m, p_m);
		}
	}

	public Individuals(ProcessTime pTime, int p, double c, double p_m) {
		mPopCount = p;
		mCrossRate = c;
		mIndivs = new Chromosome[mPopCount];
		mElite = new Chromosome(pTime, p_m);
		updated = false;

		for (int i = 0; i < mPopCount; i++) {
			mIndivs[i] = new Chromosome(pTime, p_m);
		}
	}
	 ...
	 * @param id2 親個体ID2
	 */
	public void crossover(int id1, int id2, int n, int m) {
		int length; // 遺伝子長
		...
		setOneColumn(mIndivs[id1].getGene(),
				mIndivs[id1].getTimeLine().getProcessTime().getResourceCount(), path[0]);
		setOneColumn(mIndivs[id2].getGene(),
				mIndivs[id2].getTimeLine().getProcessTime().getResourceCount(), path[1]);
		...
		for (j = 0; j < 2; j++) {
			setTwoColumn(path[j], mIndivs[id1].getTimeLine().getProcessTime().getResourceCount(),
					pmxResult[j]);
		}
	...
	public void generateNeighbor(Individuals obj) {
		int k = 0;
		int generateCount = (int) ((mPopCount - obj.getPopCount()) / (2 * obj.getPopCount()));
		...
		for (int i = 0; i < obj.getPopCount(); i++) {
			...
			for (int j = 0; j < generateCount; j++) {
				setChromosome(k, obj.getChromosome(i));
				mIndivs[k].changeJobNeighbor((int) (Math.random()
						* (obj.getChromosome(i).getTimeLine().getProcessTime().getJobCount())));
				k++;
			}
			for (int j = 0; j < generateCount; j++) {
				setChromosome(k, obj.getChromosome(i));
				mIndivs[k].changeResourceNeighbor((int) (Math.random()
						* (obj.getChromosome(i).getTimeLine().getProcessTime()
								.getResourceCount())));
				k++;
			}
		}
